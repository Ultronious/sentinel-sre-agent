...
def _get_selected_incident_with_retry() -> dict:
    ...
    Retry only the AWS alarm-history lookup.
    """
...
def handle_event(event: dict) -> dict:
    """
    Translate an EventBridge CloudWatch alarm state-change event into
    a Sentinel investigation followed by the shared review pipeline.
    """
    ...
    if not selected_incident:
        return {
            **base_result,
            ...
            "attempts": history_result.get("attempts", 1),
            "incident": None,
            "report": None,
            "history_result": history_result,
        }
...
A deterministic alarm-history lookup identified the canonical
incident.
...
Produce exactly ONE concise investigation report using the standard
Sentinel report structure.

Do not include analysis outside the final report.
"""

    try:
        initial_report = investigate(prompt)
...
"""
    ...
    return {
        **base_result,
        "status": "success",
        "message": "Sentinel investigation and review completed.",
        "attempts": history_result.get("attempts", 1),
        "incident": selected_incident,
